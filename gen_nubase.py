...
def gen_nubase():
    ...
    try:
        if os.path.exists("./onlineTool/dist/ionic_data.sqlite"):
            os.remove("./onlineTool/dist/ionic_data.sqlite")
        # create the database
        conn = sqlite3.connect('./onlineTool/dist/ionic_data.sqlite')
        c = conn.cursor()
        ...
        with open("./nubase2020.txt") as nubase:
            for _ in '_'*25:
                nubase.readline()
            for l in nubase:
                A, Z, Q, isomer_state, element, mass = int(l[:3]), int(l[4:7]), int(l[4:7]), l[7], re.split('(\d+)', l[11:16])[-1][:2], l[18:31].split()
                ...
                if len(mass) == 0:
                    ...
                    mass = A + float(mass[0][:-1]) * keV2u
                else:
                    mass = A + float(mass[0]) * keV2u
                while True:
                    if mass == " ":
                        break
                    if Z == 0 and Q == 0:
                        ...
                        break
                    c.execute("INSERT INTO IONICDATA (A,ELEMENT,Q,Z,N,ISOMERIC,TYPE,MASS,HALFLIFE)\
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (A, element, Q, Z, A-Z, isomer_state, orb_e[int(Z-Q)], _mass, half_life))
                    Q -= 1
        conn.commit()
        conn.close()
